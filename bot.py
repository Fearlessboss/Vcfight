"""
🔥 ZUDO USERBOT — GOD-LEVEL EDITION v10 (ULTRA-PARALLEL / NO-STUTTER / ZERO-CRASH)

═══════════════════════════════════════════════════════════════════════════════
NEW IN v10 (Fixes vs v9):
═══════════════════════════════════════════════════════════════════════════════
✦ FIX: 100+ userbots simultaneously → fully isolated PyTgCalls per host,
       per-(host, chat) locks, NO global semaphore choking
✦ FIX: One userbot's stop killing other userbot's stream → strict account_key
       isolation, ZERO shared state mutation between hosts
✦ FIX: Song stutter/atak-atak → ffmpeg pre-transcode to consistent format
       (48kHz stereo Opus) before streaming → smooth playback always
✦ FIX: .vplay server restart → safe video stream builder with subprocess
       isolation + automatic fallback to audio-only on video failure
✦ FIX: VC Fight + Music parallel crash → per-account locks (not just music)
✦ FIX: Bot main process crash → uvloop + signal handler + supervisor loop
✦ FIX: PyTgCalls handler errors propagating → each host's handler wrapped
       in try/except that NEVER touches other host's state
✦ FIX: MongoDB reconnect on transient failures
✦ NEW: Stream pre-buffer (download next-in-queue while current plays)
✦ NEW: Higher download parallelism (32 instead of 8)
✦ NEW: Better cookies fallback + Invidious mirror as last resort
✦ KEEP: All 40+ commands, MongoDB persistence, OTP/2FA, 2 owners
"""
...
# Try uvloop for performance (optional)
try:
    import uvloop
    uvloop.install()
except Exception:
...
# NEW v10: pre-transcode all audio to consistent format for smooth playback
ENABLE_PRE_TRANSCODE = _get_env("ENABLE_PRE_TRANSCODE", default="1", cast=int)

COOKIES_FILE = "/app/cookies/cookies.txt"
DOWNLOADS_DIR = "/tmp/downloads"

MAX_RECENT_CACHE = 50          # bigger cache for many hosts
MAX_DOWNLOAD_DIR_MB = 800       # bigger disk budget for many hosts
GC_INTERVAL_SEC = 240
WATCHDOG_INTERVAL = 30

# v10: massively increased — no global throttle on hosts
GLOBAL_DOWNLOAD_CONCURRENCY = 32
...
class _ColorFmt(logging.Formatter):
    ...
    def format(self, record):
        ...
        try:
            ...
            pass
        return f"{color}[{ts}] [{record.levelname:<7}] [ZUDO] {msg}{self.RESET}"
...
# Patch pyrogram dispatcher to swallow PeerIdInvalid silently
try:
    import pyrogram.dispatcher as _pyro_dispatcher
    _orig_handle = _pyro_dispatcher.Dispatcher.handle_updates

    async def _safe_handle_updates(self, updates, users, chats):
        try:
            return await _orig_handle(self, updates, users, chats)
        except (PeerIdInvalid, KeyError, ValueError):
            return None
        except Exception:
            return None
    _pyro_dispatcher.Dispatcher.handle_updates = _safe_handle_updates
except Exception:
    pass

# Global asyncio exception handler — NEVER crash on stray exceptions
def _global_exc_handler(loop, context):
    exc = context.get("exception")
    if isinstance(exc, (PeerIdInvalid, KeyError, ValueError, asyncio.CancelledError)):
        return
    msg = context.get("message", "")
    logger.error(f"[GlobalExc] {msg} | exc={exc}")

try:
    asyncio.get_event_loop().set_exception_handler(_global_exc_handler)
except Exception:
    pass

# Signal handlers — don't die on SIGTERM/SIGUSR1
def _signal_handler(signum, frame):
    logger.warning(f"Received signal {signum} — ignoring (use Ctrl+C twice to force exit)")
...
col_music_hosts = col_music_sudo = col_stats = None

def _init_mongo():
    global MONGO_OK, mongo_client, db
    global col_sudo, col_cache, col_sessions, col_default
    global col_music_hosts, col_music_sudo, col_stats
    try:
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🧠 IN-MEMORY STATE — STRICTLY PER-HOST ISOLATED
# ═══════════════════════════════════════════════════════════════════════════════
default_account: Client = None
default_calls: PyTgCalls = None
...
# Streams — keyed by (account_key, chat_id)
# account_key forms:
#   "default"            for default VC fight account
#   ("vc", uid)          for VC fight per-user
#   ("music", host_id)   for music userbot per-host
active_streams = {}
prejoin_state = {}
stream_owners = {}        # (account_key, chat_id) -> uid who triggered
...
# Per-(host_id OR account_key, chat_id) locks — prevents intra-chat race
play_locks = {}

# Per-account locks — prevents one user's actions blocking another
account_locks = {}

# Global download semaphore — generous to allow parallel hosts
global_play_sem = asyncio.Semaphore(GLOBAL_DOWNLOAD_CONCURRENCY)
...
def _get_play_lock(account_key, chat_id):
    """Per-(account_key, chat_id) lock so the SAME userbot in the SAME chat
    doesn't race; different userbots / different chats run fully parallel."""
...
def _get_account_lock(account_key):
    """Per-account lock — used for VERY brief critical sections like
    join/leave to avoid pytgcalls internal state races inside one client."""
    if account_key not in account_locks:
        account_locks[account_key] = asyncio.Lock()
    return account_locks[account_key]
...
def safe_handler(func):
    ...
    async def wrapper(c, m, *a, **kw):
        try:
            ...
            logger.error(
                f"[safe_handler] {func.__name__} crashed: {e}\n"
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🎵 DOWNLOAD RESOLVER — v10 with smooth-playback transcode
# ═══════════════════════════════════════════════════════════════════════════════
...
def _make_dl_opts(idx: int, out_template: str, want_video: bool = False):
    if want_video:
        # v10: more conservative video quality to avoid encoding stalls
        fmt = ('best[ext=mp4][height<=360]/best[height<=360]/'
               'bestvideo[ext=mp4][height<=360]+bestaudio[ext=m4a]/best')
        clients = _VIDEO_PLAYER_CLIENTS[idx % len(_VIDEO_PLAYER_CLIENTS)]
    else:
        fmt = 'bestaudio[ext=m4a]/bestaudio/best'
        clients = _YT_PLAYER_CLIENTS[idx % len(_YT_PLAYER_CLIENTS)]
    opts = {
        "format": fmt,
        ...
        "user_agent": YDL_UA,
        "concurrent_fragment_downloads": 16,    # v10: was 8
        "http_chunk_size": 10 * 1024 * 1024,
        "retries": 5,                           # v10: more retries
        "fragment_retries": 5,
        "socket_timeout": 25,
...
def _ydl_extract(query: str, want_video: bool):
            ...
            with yt_dlp.YoutubeDL(opts) as ydl:
                ...
                for f in os.listdir(DOWNLOADS_DIR):
                    if f.startswith(fname.split(".")[0]):
                        actual = os.path.join(DOWNLOADS_DIR, f)
                        break
                if not actual:
                    actual = out_path
                size = os.path.getsize(actual) if os.path.exists(actual) else 0
                logger.info(
                    f"✅ yt-dlp[{idx}/{clients_used}] '{title[:50]}' "
...
async def _transcode_for_smooth_playback(input_path: str, is_video: bool = False) -> str:
    """v10 NEW: Transcode downloaded media to a format that PyTgCalls/ntgcalls
    consumes WITHOUT stuttering. Audio → 48kHz stereo opus in webm.
    Video → 360p H264 + AAC mp4. If transcode fails, return original."""
    if not ENABLE_PRE_TRANSCODE:
        return input_path
    try:
        ...
        if is_video:
            ...
            out = f"{base}_TX.opus"
            # Apply volume boost in same pass if enabled
            af_chain = "aresample=48000"
            if ENABLE_AUDIO_BOOST:
                af_chain = f"volume={VOLUME_BOOST},{af_chain}"
            cmd = [
                "ffmpeg", "-y", "-hide_banner", "-loglevel", "error",
                "-i", input_path,
                "-vn",
        ...
        try:
            ...
            try:
                ...
                pass
            logger.warning(f"transcode timeout for {input_path}")
            return input_path
        if proc.returncode == 0 and os.path.exists(out) and os.path.getsize(out) > 1024:
            ...
            err_msg = (stderr or b"").decode(errors="ignore")[:200]
            logger.warning(f"transcode rc={proc.returncode} err={err_msg}")
            return input_path
    except Exception as e:
        logger.warning(f"transcode exception: {e}")
        return input_path
...
async def resolve_stream_instant(query: str, want_video: bool = False):
    ...
    if cached:
        logger.info(f"♻️ Cache hit: {cached['title'][:50]}")
        return cached["file_path"], cached["title"], cached["duration"]
    async with global_play_sem:
        logger.info(f"⚡ Downloading: {query[:80]} (video={want_video})")
        loop = asyncio.get_event_loop()
        file_path, title, duration = await loop.run_in_executor(
            None, _ydl_extract, query, want_video
        )
        # v10: smooth-playback transcode
        file_path = await _transcode_for_smooth_playback(file_path, is_video=want_video)
    _recent_cache_put(cache_key, {
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🔊 LEGACY BOOST (only used if pre-transcode is disabled)
# ═══════════════════════════════════════════════════════════════════════════════
async def boost_audio(input_path: str, gain: float = None) -> str:
    if not ENABLE_AUDIO_BOOST or ENABLE_PRE_TRANSCODE:
        # pre-transcode already applied boost
        return input_path
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🎯 STREAM BUILDER — v10 with bullet-proof video fallback
# ═══════════════════════════════════════════════════════════════════════════════
def _build_stream_objs(source: str, is_video: bool = False):
    streams = []
    if is_video:
        # Try MediaStream first (modern API)
        if _MediaStream:
            try:
                kwargs = {}
                if _AudioQuality:
                    kwargs["audio_parameters"] = _AudioQuality.HIGH
                if _VideoQuality:
                    # v10: use lowest reliable quality for video to avoid encode stalls
                    kwargs["video_parameters"] = (
                        getattr(_VideoQuality, "SD_360p", None)
                        or getattr(_VideoQuality, "HD_720p", None)
                    )
                # Remove None
                kwargs = {k: v for k, v in kwargs.items() if v is not None}
                streams.append(_MediaStream(source, **kwargs))
            except Exception as e:
                logger.debug(f"MediaStream(video) build err: {e}")
                try:
                    streams.append(_MediaStream(source))
                except Exception:
                    pass
        # Legacy AudioVideoPiped
        if _VideoPiped:
            try:
                ...
                logger.debug(f"AudioVideoPiped build err: {e}")
        # v10 FALLBACK: if no video builder, downgrade to audio-only
        # (prevents server restart when video subsystem broken)
        if not streams:
            logger.warning("No video stream builder available — falling back to audio-only")
            if _MediaStream:
                try:
                    kwargs = {}
                    if _AudioQuality:
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🎯 JOIN VC + PLAY — strict per-account isolation
# ═══════════════════════════════════════════════════════════════════════════════
...
async def join_vc_and_stream(client: Client, calls: PyTgCalls,
    ...
    """Strictly per-(account_key, chat_id) isolated. Acquires account-level
    lock ONLY for the short pytgcalls call to prevent intra-client state races,
    but does NOT block other accounts."""
    if not await ensure_peer(client, chat_id):
        return False, "Could not resolve chat peer"
    ...
    # Acquire account lock only for the actual pytgcalls call
    acc_lock = _get_account_lock(account_key)

    # Already streaming in this VC → switch stream
...
def _clear_stream_state(account_key, chat_id):
    """Clear ONLY this (account_key, chat_id) — never touch other accounts."""
    sk = (account_key, chat_id)
...
async def safe_leave(calls, chat_id):
    """Leave a group call without raising. Per-call only."""
    if not calls:
        return
    try:
        res = calls.leave_group_call(chat_id)
        if asyncio.iscoroutine(res):
            await res
    except Exception as e:
        logger.debug(f"safe_leave({chat_id}) err: {e}")


# ═══════════════════════════════════════════════════════════════════════════════
# 📡 STREAM-END EVENT HANDLER — STRICTLY ISOLATED PER HOST
# ═══════════════════════════════════════════════════════════════════════════════
def _register_stream_end_handler(calls: PyTgCalls, host_id: int, bot_client: Client):
    """The handler closure captures host_id, so it touches ONLY this host's state."""
    try:
        @calls.on_update()
        async def _on_update(_, update):
            try:
                ...
                is_end = (
                    "streamended" in evt_name
                    or "audioended" in evt_name
                    or "videoended" in evt_name
                    or "closed" in evt_name
                )
                if not is_end:
                    return
                ...
                if chat_id is None:
                    return
                # Only act if this update is for OUR host's active stream
                sk = (("music", host_id), chat_id)
                if sk not in active_streams and (host_id, chat_id) not in music_now_playing:
                    return
                logger.info(f"🔔 Stream ended: host={host_id}, chat={chat_id}")
                await _on_track_end(bot_client, host_id, chat_id)
            except Exception as e:
                logger.warning(f"on_update handler err (host={host_id}): {e}")
    except Exception as e:
...
async def auto_leave_after_playback(calls, account_key, chat_id, source, cleanup_path=None):
    ...
    try:
        ...
        if sk in active_streams:
            await safe_leave(calls, chat_id)
...
━━━━━━━ 👑  OWNER ONLY  ━━━━━━━
  ...
  /broadcast <text>   • Broadcast to all hosts
...
━━━━━━━ 🎵 MUSIC USERBOT (in groups, .) ━━━━━━━
  ...
  .auth <user>        • Grant sudo
  .dauth <user>       • Revoke sudo
  .authlist           • Sudo list
  .ping / .alive      • Liveness
...
async def _enqueue_or_play(c: Client, m: Message, host_id: int, query: str,
    ...
    if not _is_caller_allowed(host_id, m.from_user.id):
        return
    account_key = ("music", host_id)
    # v10: lock is per-(account_key, chat_id) — different hosts run in parallel,
    # same host in DIFFERENT chats also runs in parallel, ONLY same host in same chat serializes
    lock = _get_play_lock(account_key, chat_id)
    async with lock:
        proc = await m.reply_text("⚡ Searching…")
        try:
            file_path, title, duration = await resolve_stream_instant(
                query, want_video=want_video
            )
        except Exception as e:
        ...
        song = {
            ...
            "requested_by_mention": (
                requester.mention if hasattr(requester, "mention")
        ...
        if not calls:
            ...
            return
        ok, err = await join_vc_and_stream(
            c, calls, chat_id, file_path, account_key,
        ...
        if ok:
            ...
            music_now_playing.pop(key, None)
            # v10: if video failed, auto-retry as audio
            if want_video:
                logger.warning(f"Video play failed, auto-fallback to audio: {err}")
                try:
                    await proc.edit_text(f"⚠️ Video failed, trying audio…")
                except Exception:
                    pass
                ok2, err2 = await join_vc_and_stream(
                    c, calls, chat_id, file_path, account_key,
                    is_video=False, owner_uid=requester.id)
                if ok2:
                    music_now_playing[key] = song
                    song["is_video"] = False
                    await proc.edit_text(
                        f"▶️ **Now Playing (audio fallback)** 🎵\n"
                        f"**{title}**\n💬 By: {song['requested_by_mention']}")
                    return
                err = err2 or err
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🎤 MUSIC HANDLERS — host_id captured in closure so PER-HOST isolated
# ═══════════════════════════════════════════════════════════════════════════════
def register_music_handlers(client: Client, host_id: int):
    if host_id in music_handler_refs:
        return
    handler_refs = []
    ...
    async def _play(c, m: Message):
        if not _is_group_chat(m) or not m.from_user:
            return
        if len(m.command) < 2:
            await m.reply_text("Usage: `.play <song or url>`")
            return
    ...
    async def _vplay(c, m: Message):
        if not _is_group_chat(m) or not m.from_user:
            return
        if len(m.command) < 2:
            await m.reply_text("Usage: `.vplay <song or url>`")
            return
    ...
    async def _skip(c, m: Message):
        ...
        if not current and not q:
            await m.reply_text("ℹ️ Nothing to skip.")
            return
        await m.reply_text("⏭ Skipping…")
        if current:
            try:
                p = current.get("file_path")
                if p and os.path.exists(p):
    ...
    async def _stop_cmd(c, m: Message):
        """Stops ONLY this host's stream in THIS chat — never touches other hosts."""
        if not _is_group_chat(m) or not m.from_user or not _is_caller_allowed(host_id, m.from_user.id):
            return
        chat_id = m.chat.id
        calls = music_calls.get(host_id)
        ...
        if q:
            for s in q:
                try:
                    ...
                    pass
        music_loop_mode.pop((host_id, chat_id), None)
    ...
    async def _volume(c, m: Message):
        if not _is_group_chat(m) or not m.from_user or not _is_caller_allowed(host_id, m.from_user.id):
            return
        if len(m.command) < 2:
            await m.reply_text("Usage: `.volume <1-200>`")
            return
        ...
        if hasattr(calls, "change_volume_call"):
            ...
            await m.reply_text("❌ Volume API not supported.")
            return
    ...
    async def _queue_cmd(c, m: Message):
        ...
        q = _q_get(host_id, chat_id)
        if not current and not q:
            await m.reply_text("ℹ️ Queue empty.")
            return
        lines = ["🎶 **Queue:**\n"]
        if current:
            lines.append(
                f"▶️ **Now:** {current.get('title','?')} "
                f"{'🎬' if current.get('is_video') else '🎵'}"
            )
    ...
    async def _np(c, m: Message):
        ...
        if not current:
            await m.reply_text("ℹ️ Nothing playing.")
            return
        await m.reply_text(
    ...
    async def _shuffle(c, m: Message):
        ...
        if len(q) < 2:
            await m.reply_text("ℹ️ Queue too small.")
            return
        lst = list(q)
        random.shuffle(lst)
        music_queues[(host_id, m.chat.id)] = deque(lst)
        await m.reply_text(f"🔀 Shuffled **{len(lst)}** songs.")
        ...
        if q:
            for s in q:
                try:
                    ...
                    pass
            await m.reply_text(f"🗑 Cleared **{len(q)}** songs.")
        else:
            await m.reply_text("ℹ️ Queue already empty.")
    ...
    async def _replay(c, m: Message):
        ...
        current = music_now_playing.get((host_id, m.chat.id))
        if not current:
            await m.reply_text("ℹ️ Nothing to replay.")
            return
        ok, err = await join_vc_and_stream(
            c, music_calls[host_id], m.chat.id, current["file_path"],
            ("music", host_id),
            is_video=current.get("is_video", False), owner_uid=m.from_user.id)
        if ok:
            await m.reply_text("🔄 **Replaying.**")
        else:
    ...
    async def _lyrics(c, m: Message):
        ...
        if len(m.command) < 2:
            ...
            if current:
                ...
                await m.reply_text("Usage: `.lyrics <song>`")
                return
        else:
        ...
        try:
            r = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: requests.get(
                    f"https://api.lyrics.ovh/v1/{quote(query)}", timeout=15
                )
            )
    ...
    async def _search(c, m: Message):
        if not _is_group_chat(m) or not m.from_user or not _is_caller_allowed(host_id, m.from_user.id):
            return
        if len(m.command) < 2:
            await m.reply_text("Usage: `.search <query>`")
            return
        query = " ".join(m.command[1:])
        ...
        info = await asyncio.get_event_loop().run_in_executor(None, _do_search)
        entries = info.get("entries", []) if info else []
        if not entries:
            await proc.edit_text("❌ No results.")
            return
    ...
    async def _joinvc(c, m: Message):
        if not _is_group_chat(m) or not m.from_user or not _is_caller_allowed(host_id, m.from_user.id):
            return
        await m.reply_text("ℹ️ Use `.play` to join VC with audio.")
    ...
    async def _restart_inline(c, m: Message):
        ...
        if ok:
            try:
                await msg.edit_text("✅ Music userbot restarted!")
            except Exception:
                pass
        else:
            try:
    ...
    async def _sysinfo(c, m: Message):
        ...
        try:
            ...
            txt = (
                ...
                f"• RAM: `{mem.percent}%` "
                f"({mem.used//1024//1024}M / {mem.total//1024//1024}M)\n"
    ...
    async def _speedtest(c, m: Message):
        ...
        try:
            ...
            size = await asyncio.get_event_loop().run_in_executor(None, _dl)
    ...
    async def _ytdl(c, m: Message):
        ...
        if not _is_caller_allowed(host_id, m.from_user.id):
            return
        if len(m.command) < 2:
            await m.reply_text("Usage: `.ytdl <url or query>`")
            return
        q = " ".join(m.command[1:])
        proc = await m.reply_text("⏳ Resolving…")
        t0 = time.time()
        fp, title, dur = await resolve_stream_instant(q)
        el = time.time() - t0
        await proc.edit_text(
            f"✅ **{title}**\nDuration: `{dur}s`\nTime: `{el:.2f}s`\n"
    ...
    logger.info(
        f"✅ Music handlers attached for host {host_id} ({len(handler_refs)} cmds)"
    )
...
async def restart_music_userbot(host_id: int) -> bool:
    ...
    # Clear ONLY this host's state — NEVER iterate over others
    for sk in list(active_streams.keys()):
        if isinstance(sk[0], tuple) and sk[0] == ("music", host_id):
            _clear_stream_state(("music", host_id), sk[1])
    for key in list(music_queues.keys()):
        if key[0] == host_id:
            q = music_queues.pop(key, None)
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🔄 RESTORE SESSIONS — parallel, isolated
# ═══════════════════════════════════════════════════════════════════════════════
async def restore_sessions():
    ...
    if d and d.get("session_string"):
        try:
            ...
            try:
                col_default.delete_one({"_id": "default"})
            except Exception:
                pass
        except Exception as e:
    ...
    if sessions:
        # v10: higher parallel restore (5 instead of 3) for big fleets
        ...
        await asyncio.gather(
            *[_bounded(s) for s in sessions], return_exceptions=True
        )
    ...
    while True:
        try:
            ...
            if cleaned or collected:
                logger.info(
                    f"🧹 Janitor: cleaned={cleaned}, gc={collected}, "
                    f"cache={len(RECENT_DL_CACHE)}"
                )
        except asyncio.CancelledError:
            raise
        except Exception as e:
...
async def watchdog():
    """Periodically check stuck streams and recover. PER-host isolated."""
    while True:
        try:
...
async def start_command(client, m: Message):
    ...
    txt = (
        ...
        "║   🎵  **ZUDO USERBOT v10**  🎵   ║\n"
        "╚══════════════════════════════╝\n\n"
        "👋 Welcome, hacker. Choose your weapon:\n\n"
...
async def restart_cmd(c, m: Message):
    ...
    if ok:
        ...
        await proc.edit_text("❌ Restart fail. Session expire — /start se re-host karo.")
...
async def restartall_cmd(c, m: Message):
    ...
    if not hosts:
        await m.reply_text("ℹ️ No music userbots hosted.")
        return
...
async def broadcast_cmd(c, m: Message):
    if not is_owner(m.from_user.id):
        return
    if len(m.command) < 2:
        await m.reply_text("Usage: `/broadcast <text>`")
        return
...
async def stop_cmd(c, m: Message):
    """Stops ONLY the calling user's streams — never affects other users."""
    if not is_authorized(m.from_user.id):
        return
    uid = m.from_user.id
    my_streams = [sk for sk, owner in stream_owners.items() if owner == uid]
    if not my_streams:
        await m.reply_text("ℹ️ You have no active streams.")
        return
    count = 0
    ...
    try:
        ...
        if data == "use_default":
            if default_account:
                ...
                if not is_owner(uid):
                    await q.answer("❌ Default not set up yet.", show_alert=True)
                    return
        ...
        if data == "music_host":
            if uid in music_accounts:
                await q.answer("✅ Already hosted!", show_alert=True)
                return
...
async def verify_otp_text(m: Message, mode: str, otp: str):
    ...
    try:
        ...
        state.step = {
            "default_otp": "default_2fa", "custom_otp": "custom_2fa",
            "music_otp": "music_2fa"
        }[mode]
...
async def _finalize_login(uid, uc: Client, mode: str, proc_msg, state):
    ...
    try:
        await proc_msg.edit_text("✅ Verified! Setting up…")
    except Exception:
        pass
...
async def msg_handler(client, m: Message):
    ...
    if state.step in ("default_phone", "custom_phone", "music_phone"):
        ...
        try:
            name = {
                "default_phone": f"default_tmp_{uid}",
            ...
            next_mode = {
                "default_phone": "default_otp", "custom_phone": "custom_otp",
    ...
    if state.step in ("default_group", "custom_group"):
        ...
        if ci["type"] == "username":
            ...
            ok, cid, title, err, need_id = await get_chat_id_smart(
                client_to_use, ci, user_key_for_cache
            )
            if ok:
                state.data["actual_chat_id"] = cid
            ...
            try:
                ...
                await proc.edit_text("❌ Invite expired!")
                state.step = None
                return
            except Exception:
                pass
    ...
    if state.step == "audio_input":
        ...
        t0 = time.time()
        try:
            file_path, song_title, _dur = await resolve_stream_instant(text)
        except Exception as e:
            await proc.edit_text(f"❌ Download error: {e}")
            state.step = None
            return
        if not file_path:
            await proc.edit_text("❌ Couldn't fetch audio")
        ...
        # v10: VC fight also uses per-(account_key, chat_id) lock
        lock = _get_play_lock(account_key, actual_id)
        async with lock:
            ok, err = await join_vc_and_stream(
...
async def audio_handler(client, m: Message):
    ...
    # v10: always transcode user-uploaded audio for smoothness
    audio_file = await _transcode_for_smooth_playback(raw, is_video=False)
    actual = state.data.get("actual_chat_id")
    title = state.data.get("chat_title", "Group")
    await proc.edit_text("🔌 Joining VC…")
    lock = _get_play_lock(account_key, actual)
    async with lock:
        ok, err = await join_vc_and_stream(
...
def _banner():
    print("\n" + "=" * 70)
    print("🔥  ZUDO USERBOT — GOD-LEVEL v10 (ULTRA-PARALLEL • NO-STUTTER • ZERO-CRASH)")
    print("=" * 70)
    print(f"   👑 Primary Owner : {OWNER_ID}")
    print(f"   👑 Co-Owner      : {CO_OWNER_ID}")
    print(f"   🗄️  MongoDB       : {'OK' if MONGO_OK else 'FAIL'}")
    print(f"   🔊 Volume Boost  : {VOLUME_BOOST}x ({'ON' if ENABLE_AUDIO_BOOST else 'OFF'})")
    print(f"   🎚 Pre-Transcode : {'ON (smooth playback)' if ENABLE_PRE_TRANSCODE else 'OFF'}")
    print(f"   🔑 Bot Token     : {'*'*8}{BOT_TOKEN[-4:] if BOT_TOKEN else '????'}")
    print(f"   ⚡ Stream Mode   : DOWNLOAD-TRANSCODE-PLAY ({GLOBAL_DOWNLOAD_CONCURRENCY}x parallel)")
    print(f"   📥 Queue System  : per-(host, chat) deque + Lock")
    print(f"   🛑 Stop Mode     : strictly per-user / per-host (NO collisions)")
    print(f"   🎬 .vplay        : safe with auto-fallback to audio")
    ...
    print(f"   🔒 Locks         : per-account + per-(account, chat)")
...
if __name__ == "__main__":
    _banner()
    logger.info("🚀 Starting ZUDO USERBOT v10 (ultra-parallel mode)…")
    ...
    # v10: supervisor loop — if main task crashes for transient reason, restart
    while True:
        try:
            ...
            logger.error(f"Top-level crash: {e}\n{traceback.format_exc()}")
            logger.warning("Restarting main loop in 5s…")
            time.sleep(5)
            continue
