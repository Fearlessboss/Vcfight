...
# ============================================================
OWNER_ID = 7661825494
BOT_TOKEN = "7845373810:AAEjd3Z-mv68HztQRf0ZLeIiFawFUzv-KI0"
API_ID = 33628258
...
async def send_error_to_owner(msg):
    try:
...
async def restore_user_account(user_id):
    try:
        ...
        await client.start()
        user_accounts[user_id] = client
        user_calls[user_id] = PyTgCalls(client)
        await user_calls[user_id].start()
        return True
    except Exception as e:
        logger.error(f"Restore user {user_id} error: {e}")
        return False
...
async def restore_music_host(user_id):
    try:
        ...
        await client.start()
        music_userbots[user_id] = client
        music_calls[user_id] = PyTgCalls(client)
        await music_calls[user_id].start()
        await register_music_handlers(client, user_id)
        return True
    except Exception as e:
        logger.error(f"Restore music host {user_id} error: {e}")
        return False


# ============================================================
# 🔢 OTP KEYPAD (Telephone Style)
# ============================================================
def build_otp_keypad(current_otp, prefix):
    """Build telephone-style OTP keypad"""
    display = " ".join(list(current_otp)) if current_otp else "_ _ _ _ _"
    keyboard = [
        [
            InlineKeyboardButton("1", callback_data=f"{prefix}_otp_1"),
            InlineKeyboardButton("2", callback_data=f"{prefix}_otp_2"),
            InlineKeyboardButton("3", callback_data=f"{prefix}_otp_3"),
        ],
        [
            InlineKeyboardButton("4", callback_data=f"{prefix}_otp_4"),
            InlineKeyboardButton("5", callback_data=f"{prefix}_otp_5"),
            InlineKeyboardButton("6", callback_data=f"{prefix}_otp_6"),
        ],
        [
            InlineKeyboardButton("7", callback_data=f"{prefix}_otp_7"),
            InlineKeyboardButton("8", callback_data=f"{prefix}_otp_8"),
            InlineKeyboardButton("9", callback_data=f"{prefix}_otp_9"),
        ],
        [
            InlineKeyboardButton("« Back", callback_data=f"{prefix}_otp_back"),
            InlineKeyboardButton("0", callback_data=f"{prefix}_otp_0"),
            InlineKeyboardButton("✅ Submit", callback_data=f"{prefix}_otp_submit"),
        ],
        [
            InlineKeyboardButton("🗑️ Clear", callback_data=f"{prefix}_otp_clear"),
            InlineKeyboardButton("❌ Cancel", callback_data=f"{prefix}_otp_cancel"),
        ],
    ]
    return display, InlineKeyboardMarkup(keyboard)
...
async def start_command(client, message: Message):
    try:
        ...
        await send_error_to_owner(f"Start error: {e}")
...
async def menu_callback(client, cq: CallbackQuery):
    ...
    if data == "menu_vcfight":
        ...
        keyboard_buttons = [
            [InlineKeyboardButton("🔵 Default Account", callback_data="vc_default")],
        ]
        if user_id in user_accounts:
            keyboard_buttons.append([InlineKeyboardButton("🎵 Play Audio (My Account)", callback_data="vc_custom_play")])
            keyboard_buttons.append([InlineKeyboardButton("🚪 Logout My Account", callback_data="vc_logout")])
        else:
            keyboard_buttons.append([InlineKeyboardButton("🟢 Login My Account", callback_data="vc_custom_login")])

        keyboard_buttons.append([InlineKeyboardButton("« Back", callback_data="menu_back")])
        ...
        # Check if user has hosted music userbot
        if user_id not in music_userbots:
            await restore_music_host(user_id)

        if user_id in music_userbots:
            kb = InlineKeyboardMarkup([
                [InlineKeyboardButton("✅ Already Hosted", callback_data="music_status")],
                [InlineKeyboardButton("📜 Commands List", callback_data="music_commands")],
                [InlineKeyboardButton("🚪 Logout Music Bot", callback_data="music_logout")],
                [InlineKeyboardButton("« Back", callback_data="menu_back")],
            ])
            await cq.message.edit_text(
                "🎵 **Music Userbot Menu**\n\n"
                "✅ **Your music userbot is already hosted!**\n\n"
                "Use **/commands** anywhere or click 📜 button to see all music commands.\n\n"
                "💡 Go to any group and type `.play <song>` to play music.",
                reply_markup=kb
            )
        else:
            kb = InlineKeyboardMarkup([
...
# ============================================================
@bot.on_callback_query(filters.regex("^(vclogin|musichost|setdef)_otp_"))
async def otp_callback(client, cq: CallbackQuery):
    global default_account, default_calls
    user_id = cq.from_user.id
    state = get_user_state(user_id)
    data = cq.data
    parts = data.split("_otp_")
    prefix = parts[0]
    action = parts[1]
...
async def process_otp(client, cq, prefix, otp):
    ...
    processing_msg = await cq.message.edit_text(f"⏳ Verifying OTP `{otp}`...")

    try:
        user_client = state.data.get("client")
        if not user_client:
            await processing_msg.edit_text("❌ Session expired! Please start again.")
            state.step = None
            return
        ...
        if prefix == "setdef":
            ...
            await processing_msg.edit_text(
                "✅ **Music Userbot hosted successfully!**\nSession saved to MongoDB.\n\n"
                "🎵 Now go to any group and use:\n"
                "`.play <song name>` `.skip` `.pause` etc.\n\n"
...
async def add_music_sudo(client, message: Message):
    try:
        if len(message.command) < 2:
            await message.reply_text("❌ Usage: `/msudo <username/userid>`")
            return
        user_input = message.command[1]
        if user_input.startswith('@'):
            user = await client.get_users(user_input[1:])
...
async def rm_music_sudo(client, message: Message):
    try:
        if len(message.command) < 2:
            await message.reply_text("❌ Usage: `/rmmsudo <username/userid>`")
            return
        user_input = message.command[1]
        if user_input.startswith('@'):
            user = await client.get_users(user_input[1:])
...
def get_music_commands_text():
    return (
        ...
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        "🎶 **PLAYBACK**\n\n"
        "▶️  `.play <song name / URL>`  —  Play a song\n"
        ...
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        "👥 **PERMISSIONS**\n\n"
        "✅  `.auth <reply/user/id>`  —  Add sudo user\n"
        "❌  `.dauth <reply/user/id>`  —  Remove sudo user\n"
        ...
        try:
            ...
            if music_now_playing[host_id].get(chat_id):
                ...
                music_queues[host_id].setdefault(chat_id, []).append({
                    "url": url, "title": title, "path": audio_path, "user": m.from_user.first_name
                })
