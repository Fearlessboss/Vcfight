"""
🔥 ZUDO USERBOT — GOD-LEVEL EDITION v3 (FULLY FIXED + UPGRADED)
==================================================================
✦ FIX: Music userbot commands (.play .skip .ping etc) ab 100% kaam karte hain
✦ FIX: Invalid/dead session ko auto-skip karta hai aur cleanup karta hai
✦ FIX: /restart command — host apna userbot fresh restart kar sakta hai
✦ FIX: VC fight default account me song play instant hota hai
✦ FIX: VC join speed boosted — parallel download + warmup
✦ FIX: AUTH_KEY_UNREGISTERED handle — purani session db se delete
✦ FIX: Filter chat type ("private" -> ChatType.PRIVATE) sahi se
✦ FIX: Music handlers per-host ab properly register & dispatch hote hain
✦ FIX: 2FA verification real-time
✦ FIX: BOT_TOKEN env var
✦ FIX: 2 owners support
✦ MongoDB persistent memory
✦ Loud audio boost (4x default)
...
async def join_vc_and_play(client: Client, calls: PyTgCalls,
                           chat_id: int, audio_path: str, stream_key):
    async def _try_join():
...
def _is_group_chat(m: Message) -> bool:
    """Properly detect group/supergroup chat using enum (not string)."""
    try:
...
def register_music_handlers(client: Client, host_id: int):
    ...
    Stores handler refs for clean removal on restart.
    """
    # Cleanup existing handlers for this host first (idempotent)
    ...
    async def _play(c, m: Message):
        try:
            ...
            async with lock:
                ...
                # Parallel: download + warmup peers (speeds up VC join)
                dl_task = asyncio.create_task(download_youtube_audio(query))
                ...
                audio_path, title = await dl_task
                # Don't block on warmup but make sure it ran enough
                try:
                    await asyncio.wait_for(warm_task, timeout=0.1)
                except asyncio.TimeoutError:
                    pass  # let it continue in background
...
def unregister_music_handlers(client: Client, host_id: int):
    """Remove previously-registered music handlers (for restart)."""
...
async def restart_music_userbot(host_id: int) -> bool:
    """
    Cleanly stop current music userbot (client+calls+handlers),
    then re-create from saved session in Mongo.
    """
    logger.info(f"♻️ Restart requested for host {host_id}")

    # 1) Stop existing PyTgCalls
    if host_id in music_calls:
        try:
            await music_calls[host_id].stop()
        except Exception as e:
            logger.warning(f"music_calls.stop err: {e}")
        music_calls.pop(host_id, None)

    # 2) Remove handlers and stop client
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🔄 RESTORE SESSIONS (with smart skip of dead sessions)
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    async def _restore_one(s):
        ...
        try:
            ...
            if kind == "music":
                ...
                UserDeactivated, UserDeactivatedBan) as e:
            # Dead session — skip and cleanup so it doesn't keep failing
            ...
            # Non-fatal — log and skip this one, continue with others
            logger.error(f"⚠️ Restore fail for {uid} ({kind}): {e} — SKIPPING")

    if sessions:
        # gather with return_exceptions so one bad session can't kill the rest
...
async def restart_cmd(c, m: Message):
    """Host apna music userbot DM se restart kar sake."""
    uid = m.from_user.id
...
async def msg_handler(client, m: Message):
    ...
    try:
        ...
        if state.step == "audio_input":
            ...
            # PARALLEL: download audio AND warm peers — speeds up VC join a LOT
            dl_task = asyncio.create_task(download_youtube_audio(text))
            warm_task = asyncio.create_task(warmup_peers(client_to_use, f"vcfight-{actual_id}"))

            audio_path, song_title = await dl_task
            try:
                await asyncio.wait_for(warm_task, timeout=0.1)
            except asyncio.TimeoutError:
                pass
...
def _banner():
    print("\n" + "="*68)
    print("🔥  ZUDO USERBOT — GOD-LEVEL EDITION v3")
    print("="*68)
...
if __name__ == "__main__":
    _banner()
    logger.info("🚀 Starting ZUDO USERBOT v3 …")
