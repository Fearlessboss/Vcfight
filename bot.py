...
async def menu_callback(client, cq: CallbackQuery):
    ...
    if data == "menu_vcfight":
        ...
        rows = []
        if user_id == OWNER_ID:
            rows.append([InlineKeyboardButton("⚡ Use Default Account", callback_data="vc_default")])
        rows.append([InlineKeyboardButton("🔐 Login with Your Account", callback_data="vc_custom_login")])
        rows.append([InlineKeyboardButton("▶️ Play (Custom Account)", callback_data="vc_custom_play")])
        rows.append([InlineKeyboardButton("🚪 Logout", callback_data="vc_logout")])
...
async def startup():
    ...
    async for doc in sessions_col.find({"type": "custom"}):
        try:
            ...
            logger.error(f"Failed restoring user {doc.get('user_id')}: {e}")
    # Restore all music hosts
    async for doc in music_hosts_col.find({}):
        try:
            await restore_music_host(doc["user_id"])
        except Exception as e:
            logger.error(f"Failed restoring music host {doc.get('user_id')}: {e}")
...
async def main():
    ...
    # ✅ FIX: Use pyrogram.idle() to properly keep the bot running
    # and ensure update dispatcher stays active
    await idle()
