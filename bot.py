"""
🔥 ZUDO USERBOT — GOD-LEVEL EDITION v4 (INSTANT STREAM + TEXT OTP)
==================================================================
✦ NEW: OTP via TEXT (space separated: "1 2 3 4 5")  — no keypad
✦ NEW: 2FA via TEXT — same flow
✦ NEW: INSTANT STREAM — VC join + play in <5s (no full download wait)
✦ NEW: yt-dlp direct stream URL extract → AudioPiped(stream_url) → instant
✦ NEW: .play command message auto-deletes after stream starts
✦ FIX: 403 Forbidden — better yt-dlp opts (UA, cookies path, retries, fmt fallback)
✦ FIX: nubcoder direct URL stream (no disk write) — instant
✦ KEEP: Music userbot, restart, 2 owners, MongoDB persistence, volume boost
==================================================================
"""
...
COOKIES_FILE = "/app/cookies/cookies.txt"  # optional — if you mount yt cookies here
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🎵 INSTANT STREAM RESOLVER — No full download, get direct URL only
# ═══════════════════════════════════════════════════════════════════════════════
YDL_UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"

def _build_ydl_opts(extract_only=True):
    """Optimized yt-dlp opts: no download, just direct stream URL extraction."""
    opts = {
...
async def resolve_stream_instant(query: str):
    """
    INSTANT resolver — returns (stream_url_or_path, title) ASAP.
    NO full download. Just gets direct playable URL from YouTube/nubcoder.
    """
    ...
    # 2) yt-dlp extract_info (no download!) — gets direct CDN URL
    try:
        loop = asyncio.get_event_loop()
        ydl_opts = _build_ydl_opts(extract_only=True)
        ...
        if info:
            ...
            # Direct URL — works with FFmpeg/AudioPiped
            ...
            if not stream_url and 'formats' in info:
                ...
                for f in reversed(info['formats']):
                    if f.get('acodec') and f.get('acodec') != 'none' and f.get('url'):
                        ...
                        break
            if stream_url:
                logger.info(f"✅ yt-dlp stream URL: {title}")
                return stream_url, title
    except Exception as e:
        logger.error(f"yt-dlp resolve err: {e}")

    return None, None


# ═══════════════════════════════════════════════════════════════════════════════
# 🎵 AUDIO BOOST (only for uploaded files — NOT for streaming)
# ═══════════════════════════════════════════════════════════════════════════════
...
async def auto_leave_after_playback(calls, stream_key, chat_id, source):
    try:
        ...
        if dur > 0:
            ...
            await asyncio.sleep(300)  # fallback
        try:
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🎯 JOIN VC + PLAY (instant streaming version)
# ═══════════════════════════════════════════════════════════════════════════════
async def join_vc_and_stream(client: Client, calls: PyTgCalls,
                             chat_id: int, source_url_or_path: str, stream_key):
    """AudioPiped works with both local files AND direct stream URLs (FFmpeg handles HTTP)."""
    async def _try_join():
        await calls.join_group_call(
            chat_id,
            AudioPiped(source_url_or_path, HighQualityAudio()),
    ...
    try:
        ...
        logger.warning(f"⚠️ peer issue ({e}); warming & retry")
        ...
        if any(x in emsg for x in [
            ...
            return False, f"⚠️ No active VC in group. Start a voice chat first."
        return False, emsg


# ═══════════════════════════════════════════════════════════════════════════════
# 🛡️ Safe stop wrapper — works across pytgcalls versions
# ═══════════════════════════════════════════════════════════════════════════════
async def safe_stop_calls(calls):
    """PyTgCalls API varies — try multiple shutdown methods."""
    if not calls:
        return
    for method in ("stop", "_stop", "terminate", "close"):
        ...
        if fn:
            try:
                ...
                continue
    # last resort — just clear active streams
    return
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🎵 MUSIC USERBOT HANDLERS (.play is now INSTANT + auto-delete cmd)
# ═══════════════════════════════════════════════════════════════════════════════
...
def register_music_handlers(client: Client, host_id: int):
    ...
    async def _play(c, m: Message):
        try:
            ...
            if lock.locked():
                # already playing, skip silently
                return

            # 🔥 DELETE .play command msg immediately
            asyncio.create_task(_safe_delete(m))

            async with lock:
                proc = await c.send_message(chat_id, f"⚡ Searching & streaming: `{query}`")

                # Parallel: instant stream resolve + peer warmup
                resolve_task = asyncio.create_task(resolve_stream_instant(query))
                asyncio.create_task(warmup_peers(c, f"play-{chat_id}"))

                try:
                    stream_url, title = await asyncio.wait_for(resolve_task, timeout=12)
                except asyncio.TimeoutError:
                ...
                ok, err = await join_vc_and_stream(c, calls, chat_id, stream_url, f"music_{host_id}")
                if not ok:
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🔘 CALLBACKS  (OTP keypad callbacks REMOVED)
# ═══════════════════════════════════════════════════════════════════════════════
...
def _clean_otp_text(text: str) -> str:
    """Accepts '1 2 3 4 5', '12345', '1-2-3-4-5' etc. → '12345'"""
...
# ═══════════════════════════════════════════════════════════════════════════════
# 💬 TEXT MESSAGE HANDLER (handles phone, OTP-text, 2FA-text, group, audio query)
# ═══════════════════════════════════════════════════════════════════════════════
...
async def msg_handler(client, m: Message):
    ...
    try:
        ...
        # VC fight audio input (now uses INSTANT stream resolver)
        if state.step == "audio_input":
            ...
            # parallel: stream resolve + warmup
            resolve_task = asyncio.create_task(resolve_stream_instant(text))
            asyncio.create_task(warmup_peers(client_to_use, f"vcfight-{actual_id}"))

            try:
                stream_url, song_title = await asyncio.wait_for(resolve_task, timeout=12)
            except asyncio.TimeoutError:
            ...
            ok, err = await join_vc_and_stream(client_to_use, calls_to_use, actual_id, stream_url, stream_key)
            if ok:
                asyncio.create_task(auto_leave_after_playback(calls_to_use, stream_key, actual_id, stream_url))
                await proc.edit_text(
                    f"✅ **NOW STREAMING** 🔥\n\n📻 Group: {title}\n🎵 {song_title or 'Audio'}\n"
                    f"⚡ Direct stream (no download)\n\nUse /stop to stop")
            else:
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🎵 AUDIO / VOICE FILE HANDLER (uploaded files — still uses boost+download)
# ═══════════════════════════════════════════════════════════════════════════════
...
async def audio_handler(client, m: Message):
    ...
    ok, err = await join_vc_and_stream(cu, calls, actual, boosted, sk)
    if ok:
...
def _banner():
    print("\n" + "="*68)
    print("🔥  ZUDO USERBOT — GOD-LEVEL EDITION v4 (INSTANT STREAM)")
    print("="*68)
    ...
    print(f"   ⚡ Stream Mode   : DIRECT URL (no full download)")
    print("="*68 + "\n")
