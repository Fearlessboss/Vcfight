"""
🔥 ZUDO USERBOT — GOD-LEVEL EDITION v9 (HACKER-MODE / ULTRA-STABLE / NO-CRASH)

✦ FIX: PeerIdInvalid / KeyError 'ID not found' → handler-wrapped, auto re-resolve via get_chat, never propagates
✦ FIX: Server restart due to unhandled exceptions → safe_handler decorator on EVERY command + global exception swallow
✦ FIX: Song stops mid-way → stream-end event hook + watchdog timer fallback (auto next from queue)
✦ FIX: Userbot doesn't leave VC after song ends → on_stream_end auto-leaves if queue empty
✦ FIX: Two users playing simultaneously crashed → per-(host, chat) asyncio.Lock + isolated account_key
✦ FIX: MongoDB default account not reloading → robust restore with retry, dead-session pruning
✦ FIX: OOM exit 137 → aggressive cleanup, gc, small cache, no boost re-encode by default
✦ FIX: 10-11 userbots simultaneously → semaphore-bounded startup, isolated PyTgCalls per host
✦ FIX: .help commands not working → all 40+ commands tested with safe_handler wrapper
✦ NEW: .speedtest .logs .reload .ytdl .clean .restart .leaveall .stats etc
✦ NEW: instant-play (resolve URL → play directly without full download where possible)
✦ NEW: parallel download + join VC to reduce play time
✦ KEEP: MongoDB persistence, OTP/2FA text input, 2 owners, peer warmup, MediaStream/AudioPiped fallback
"""
...
MAX_RECENT_CACHE     = 25
MAX_DOWNLOAD_DIR_MB  = 400
GC_INTERVAL_SEC      = 240
WATCHDOG_INTERVAL    = 30   # check stream health every 30s
...
class _ColorFmt(logging.Formatter):
    ...
    def format(self, record):
        ...
        # Push plain version to buffer for .logs
        try:
...
# Silence noisy libs so they don't crash bot
...
try:
    ...
    async def _safe_handle_updates(self, updates, users, chats):
        try:
            ...
            # These are spurious and recoverable — don't kill the loop
            return None
        except Exception:
            return None
...
# Global asyncio exception handler — don't crash on stray exceptions
def _global_exc_handler(loop, context):
    exc = context.get("exception")
    if isinstance(exc, (PeerIdInvalid, KeyError, ValueError)):
        return
...
global_play_sem = asyncio.Semaphore(8)   # max 8 concurrent downloads
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🛡 SAFE HANDLER DECORATOR (prevents any handler from killing the bot)
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# 💾 MONGO HELPERS (all wrapped to never raise)
# ═══════════════════════════════════════════════════════════════════════════════
def _safe_mongo(fn):
    @functools.wraps(fn)
    def w(*a, **kw):
        if not MONGO_OK:
            return fn._default if hasattr(fn, "_default") else None
        try:
            return fn(*a, **kw)
        except Exception as e:
            ...
            with yt_dlp.YoutubeDL(opts) as ydl:
                ...
                for f in os.listdir(DOWNLOADS_DIR):
                    if f.startswith(fname.split(".")[0]):
                        actual = os.path.join(DOWNLOADS_DIR, f); break
    ...
    if is_video:
        if _MediaStream:
            try:
                ...
                if _VideoQuality: kwargs["video_parameters"] = _VideoQuality.HD_720p
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🎯 JOIN VC + PLAY (robust, isolated per account_key)
# ═══════════════════════════════════════════════════════════════════════════════
...
def register_music_handlers(client: Client, host_id: int):
    ...
    async def _play(c, m: Message):
        ...
        if len(m.command) < 2:
            await m.reply_text("Usage: `.play <song or url>`"); return
    ...
    async def _vplay(c, m: Message):
        if not _is_group_chat(m) or not m.from_user: return
        if len(m.command) < 2:
            await m.reply_text("Usage: `.vplay <song or url>`"); return
        await _enqueue_or_play(c, m, host_id, " ".join(m.command[1:]), True, False)

    @safe_handler
    async def _playforce(c, m: Message):
        if not _is_group_chat(m) or not m.from_user or len(m.command) < 2: return
    ...
    async def _skip(c, m: Message):
        ...
        if not current and not q:
            await m.reply_text("ℹ️ Nothing to skip."); return
        ...
        if not q:
            ...
            _clear_stream_state(("music", host_id), chat_id)
            await m.reply_text("⏹ Queue empty — left VC."); return
        await _play_next_from_queue(c, host_id, chat_id)
    ...
    async def _shuffle(c, m: Message):
        ...
        if len(q) < 2:
            await m.reply_text("ℹ️ Queue too small."); return
    ...
    async def _replay(c, m: Message):
        ...
        ok, err = await join_vc_and_stream(
            c, music_calls[host_id], m.chat.id, current["file_path"], ("music", host_id),
            is_video=current.get("is_video", False), owner_uid=m.from_user.id)
        if ok: await m.reply_text("🔄 **Replaying.**")
        else:  await m.reply_text(f"❌ {err}")
    ...
    async def _lyrics(c, m: Message):
        ...
        if len(m.command) < 2:
            current = music_now_playing.get((host_id, m.chat.id))
            if current: query = current.get("title", "")
            else:
    ...
    async def _ytdl(c, m: Message):
        ...
        if not _is_caller_allowed(host_id, m.from_user.id): return
        if len(m.command) < 2:
            await m.reply_text("Usage: `.ytdl <url or query>`"); return
        q = " ".join(m.command[1:])
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🔄 RESTORE SESSIONS (robust, parallel, bounded)
# ═══════════════════════════════════════════════════════════════════════════════
async def restore_sessions():
    ...
    if sessions:
        sem = asyncio.Semaphore(3)  # max 3 parallel restores
    ...
    while True:
        try:
            ...
            if cleaned or collected:
                logger.info(f"🧹 Janitor: cleaned={cleaned}, gc={collected}, cache={len(RECENT_DL_CACHE)}")
        except Exception as e:
            logger.warning(f"Janitor error: {e}")

async def watchdog():
    """Periodically check stuck streams and recover."""
    while True:
        try:
...
# ═══════════════════════════════════════════════════════════════════════════════
# 📨 BOT COMMANDS (DM) — all safe_handler wrapped
# ═══════════════════════════════════════════════════════════════════════════════
...
def _banner():
    print("\n" + "="*70)
    print("🔥  ZUDO USERBOT — GOD-LEVEL v9 (HACKER MODE • NO-CRASH • 40+ CMDS)")
    print("="*70)
    ...
    print(f"   ⚡ Stream Mode   : DOWNLOAD-FIRST (8x parallel + cache)")
...
if __name__ == "__main__":
    _banner()
    logger.info("🚀 Starting ZUDO USERBOT v9 (hacker-mode)…")
