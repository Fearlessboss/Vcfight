"""
🔥 ZUDO USERBOT — GOD-LEVEL EDITION v8 (ULTRA-STABLE + 50+ COMMANDS)

✦ FIX: OOM (exit 137) — aggressive memory cleanup, smaller buffers,
       no audio boost re-encode by default, file cache size limits,
       gc.collect() periodic, no long-lived /tmp accumulation
✦ FIX: Song randomly stops — stream-end now uses PyTgCalls update events
       (StreamAudioEnded / Update) instead of fragile sleep(duration)
✦ FIX: Session expired bug — login state now persisted in dict with
       proper TTL + re-attach Client if disconnected
✦ FIX: Music userbot needed restart to work — calls.start() now properly
       awaited + handlers attach AFTER calls ready + warmup peers BEFORE
       registering handlers
✦ NEW: /restartall (owner only) — restart all hosted music userbots
✦ NEW: 50+ working commands — see HELP_TEXT below
✦ NEW: Per-chat queue, per-user stop isolation, .vplay video stream,
       8x parallel yt-dlp fragments, in-memory recent cache
✦ KEEP: MongoDB persistence, OTP/2FA text input, 2 owners, peer warmup,
        MediaStream/AudioPiped fallback chain, auto-cleanup
...
_StreamAudioEnded = None
_StreamVideoEnded = None
_UpdateStream = None
...
# Stream end event imports (varies by pytgcalls version)
try:
    from pytgcalls.types import StreamAudioEnded as _StreamAudioEnded
except Exception:
    pass
try:
    from pytgcalls.types import StreamVideoEnded as _StreamVideoEnded
except Exception:
    pass
try:
    from pytgcalls.types import Update as _UpdateStream
except Exception:
    pass
...
ENABLE_AUDIO_BOOST = _get_env("ENABLE_AUDIO_BOOST", default="0", cast=int)  # 0 = off (saves RAM)

COOKIES_FILE  = "/app/cookies/cookies.txt"
DOWNLOADS_DIR = "/tmp/downloads"

# Memory limits
MAX_RECENT_CACHE  = 30          # max items in RECENT_DL_CACHE
MAX_DOWNLOAD_DIR_MB = 500       # max total size of downloads dir
GC_INTERVAL_SEC   = 300         # garbage collect every 5 min
...
class _ColorFmt(logging.Formatter):
    ...
    def format(self, record):
        ...
        return (
            f"{color}[{ts}] [{record.levelname:<7}] [ZUDO] "
...
warmed_chats   = {}     # client_id -> {done, count}

# Streams
active_streams = {}     # (account_key, chat_id) -> chat_id
prejoin_state  = {}     # (account_key, chat_id) -> True
stream_owners  = {}     # (account_key, chat_id) -> uid (who started it)

# Music queue system
music_queues       = {} # (host_id, chat_id) -> deque of song dicts
music_now_playing  = {} # (host_id, chat_id) -> song dict
play_locks         = {} # (account_key, chat_id) -> asyncio.Lock

# Download cache (small to avoid OOM)
RECENT_DL_CACHE = {}    # query_lower -> {file_path, title, duration, ts}
RECENT_DL_TTL   = 30 * 60  # 30 min (reduced to save RAM)
...
def mongo_save_sudo(uid):
    if MONGO_OK:
        try: col_sudo.update_one({"_id": uid}, {"$set": {"_id": uid}}, upsert=True)
        except: pass

def mongo_remove_sudo(uid):
    if MONGO_OK:
        try: col_sudo.delete_one({"_id": uid})
        except: pass
...
def mongo_save_cache(key, chat_id, title):
    if MONGO_OK:
        try: col_cache.update_one({"_id": key}, {"$set": {"chat_id": chat_id, "title": title}}, upsert=True)
        except: pass
...
def mongo_save_music_host(uid):
    if MONGO_OK:
        try: col_music_hosts.update_one({"_id": uid}, {"$set": {"_id": uid}}, upsert=True)
        except: pass

def mongo_remove_music_host(uid):
    if MONGO_OK:
        try: col_music_hosts.delete_one({"_id": uid})
        except: pass
...
def mongo_remove_music_sudo(host_id, sudo_id):
    if MONGO_OK:
        try: col_music_sudo.delete_one({"_id": f"{host_id}:{sudo_id}"})
        except: pass
...
def _make_dl_opts(idx: int, out_template: str, want_video: bool = False):
    ...
    opts = {
        ...
        "prefer_ffmpeg": True,
        # disable post-processing (saves RAM)
        "postprocessors": [],
    }
...
def _yt_search_first(query: str, want_video: bool = False):
    """Use ytsearch1: prefix for fast top-result resolution."""
    if _is_url(query):
        return query
...
def _ydl_extract(query: str, want_video: bool):
    ...
    for idx in range(len(clients_list)):
        opts, clients_used = _make_dl_opts(idx, out_template, want_video=want_video)
        try:
            with yt_dlp.YoutubeDL(opts) as ydl:
                ...
                logger.info(
                    f"✅ yt-dlp[{idx}/{clients_used}] downloaded "
                    f"'{title[:60]}' ({size/1024/1024:.1f}MB, video={want_video}) → {actual}"
                )
...
async def resolve_stream_instant(query: str, want_video: bool = False):
    """Async wrapper for yt-dlp download — uses cache + executor."""
    cached = _recent_cache_get(query + ("|v" if want_video else "|a"))
    if cached:
        logger.info(f"♻️ Cache hit: {cached['title'][:60]}")
        return cached["file_path"], cached["title"], cached["duration"]

    logger.info(f"⚡ Resolving + downloading: {query[:80]} (video={want_video})")
    loop = asyncio.get_event_loop()
    file_path, title, duration = await loop.run_in_executor(
        None, _ydl_extract, query, want_video
    )
    _recent_cache_put(query + ("|v" if want_video else "|a"), {
        "file_path": file_path,
        "title": title,
        "duration": duration,
    })
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🔊 AUDIO BOOST (DISABLED by default to save RAM)
# ═══════════════════════════════════════════════════════════════════════════════
...
def _build_stream_objs(source: str, is_video: bool = False):
    """Return list of stream objects in order of preference."""
    streams = []
    if is_video:
        # Try MediaStream with video
        if _MediaStream:
            try:
                kwargs = {}
                if _AudioQuality:
        ...
        if _MediaStream:
            try:
                ...
                if _AudioQuality:
                    kwargs["audio_parameters"] = _AudioQuality.HIGH
                # Force audio-only flag if available
                streams.append(_MediaStream(source, video_flags=getattr(_MediaStream, "Flags", None) and _MediaStream.Flags.IGNORE if False else None, **kwargs) if False else _MediaStream(source, **kwargs))
            except Exception:
                try:
                    streams.append(_MediaStream(source))
                except: pass
...
async def join_vc_and_stream(client: Client, calls: PyTgCalls,
    ...
    """Per-user / per-chat isolated VC streaming."""
    await ensure_peer(client, chat_id)

    streams = _build_stream_objs(source, is_video)
    if not streams:
        return False, "No stream builder available (pytgcalls install broken?)"

    sk = (account_key, chat_id)

    # Already in this VC with this account → just switch stream
...
# ═══════════════════════════════════════════════════════════════════════════════
# 📡 STREAM-END EVENT HANDLER (for music queue auto-next)
# ═══════════════════════════════════════════════════════════════════════════════
def _register_stream_end_handler(calls: PyTgCalls, host_id: int, bot_client: Client):
    """Register update handler so when track ends, we play next from queue."""
    try:
        @calls.on_update()
        async def _on_update(_, update):
            try:
                # Detect stream-ended events across pytgcalls versions
                ...
                if chat_id is None:
                    return
                logger.info(f"🔔 Stream ended event for host={host_id}, chat={chat_id}")
                # Play next song from queue
                await _on_track_end(bot_client, host_id, chat_id)
            except Exception as e:
                logger.error(f"on_update handler err: {e}")
    except Exception as e:
        logger.warning(f"Could not register stream-end handler for {host_id}: {e}")


# ═══════════════════════════════════════════════════════════════════════════════
# 👋 AUTO-LEAVE (for VC Fight non-queue use)
# ═══════════════════════════════════════════════════════════════════════════════
async def auto_leave_after_playback(calls, account_key, chat_id, source, cleanup_path=None):
    ...
    try:
        ...
        if dur > 0:
            # Wait a bit extra for buffering
            await asyncio.sleep(dur + 5)
        else:
            await asyncio.sleep(300)
        # Only leave if still our stream
...
━━━━━━━ 👑  OWNER ONLY  ━━━━━━━
  ...
  /restartall         • Restart ALL hosted music userbots
...
━━━━━━━ 🎵 MUSIC USERBOT (in groups, dot-prefix) ━━━━━━━
  .play <song/url>    • Play audio (queue if busy)
  .vplay <song/url>   • Play video (queue if busy)
  .playforce <song>   • Force play (clears queue)
  ...
  .search <query>     • Search song

  .auth <user>        • Grant sudo on userbot
...
async def _on_track_end(c: Client, host_id: int, chat_id: int):
    """Called when a track ends naturally. Plays next from queue."""
    key = (host_id, chat_id)
    current = music_now_playing.get(key)

    # Loop mode: re-add current to front
...
async def _play_next_from_queue(c: Client, host_id: int, chat_id: int):
    """Pop next song from queue and stream it."""
    q = _q_get(host_id, chat_id)
    if not q:
...
async def _enqueue_or_play(c: Client, m: Message, host_id: int, query: str,
    ...
    """Add song to queue OR play immediately if nothing playing."""
    chat_id = m.chat.id
    if not _is_caller_allowed(host_id, m.from_user.id):
        return

    proc = await m.reply_text("⏳ Searching & downloading…")
    try:
        file_path, title, duration = await resolve_stream_instant(query, want_video=want_video)
    except Exception as e:
        await proc.edit_text(f"❌ Download failed: {e}")
        return
    ...
    song = {
        ...
        "requested_by_mention": requester.mention if hasattr(requester, "mention") else f"@{requester.username}" if requester.username else requester.first_name,
        "added_at": time.time(),
    }
    ...
    if force:
        # Clear queue, stop current, play this
        q = _q_get(host_id, chat_id)
        for s in q:
            try:
                p = s.get("file_path")
    ...
    if music_now_playing.get(key) and not force:
        ...
        await proc.edit_text(
            ...
            f"🎵 {title}\n"
            f"💬 By: {song['requested_by_mention']}"
        )
        return
...
def register_music_handlers(client: Client, host_id: int):
    """Attach all .commands to the music userbot client."""
    ...
    async def _play(c, m: Message):
        ...
        try:
            if len(m.command) < 2:
                try: await m.reply_text("Usage: `.play <song name or url>`")
                except: pass
                return
            query = " ".join(m.command[1:])
            await _enqueue_or_play(c, m, host_id, query, want_video=False, force=False)
        except Exception as e:
    ...
    async def _vplay(c, m: Message):
        ...
        try:
            if len(m.command) < 2:
                try: await m.reply_text("Usage: `.vplay <song name or url>`")
                except: pass
                return
            query = " ".join(m.command[1:])
            await _enqueue_or_play(c, m, host_id, query, want_video=True, force=False)
        except Exception as e:
    ...
    async def _playforce(c, m: Message):
        ...
        query = " ".join(m.command[1:])
        await _enqueue_or_play(c, m, host_id, query, want_video=False, force=True)

    async def _vplayforce(c, m: Message):
        if not _is_group_chat(m) or not m.from_user: return
        if len(m.command) < 2: return
        query = " ".join(m.command[1:])
        await _enqueue_or_play(c, m, host_id, query, want_video=True, force=True)
    ...
    async def _volume(c, m: Message):
        ...
        try:
            ...
            if hasattr(calls, "change_volume_call"):
                ...
                await m.reply_text("❌ Volume API not supported in this pytgcalls version.")
                return
    ...
    async def _shuffle(c, m: Message):
        ...
        if len(q) < 2:
            await m.reply_text("ℹ️ Queue too small to shuffle."); return
        lst = list(q)
        random.shuffle(lst)
    ...
    async def _clearqueue(c, m: Message):
        ...
        if q:
            ...
            await m.reply_text(f"🗑 Cleared **{len(q)}** songs from queue.")
        else:
    ...
    async def _replay(c, m: Message):
        ...
        current = music_now_playing.get((host_id, m.chat.id))
        if not current:
            await m.reply_text("ℹ️ Nothing to replay."); return
        try:
            calls = music_calls[host_id]
            ok, err = await join_vc_and_stream(
            ...
            if ok:
                await m.reply_text("🔄 **Replaying current track.**")
            else:
                await m.reply_text(f"❌ {err}")
        except Exception as e:
            await m.reply_text(f"❌ {e}")

    async def _seek(c, m: Message):
        if not _is_group_chat(m) or not m.from_user or not _is_caller_allowed(host_id, m.from_user.id): return
        await m.reply_text("ℹ️ Seek not implemented yet (depends on pytgcalls version).")
    ...
    async def _lyrics(c, m: Message):
        ...
        if len(m.command) < 2:
            ...
            if current:
                ...
                await m.reply_text("Usage: `.lyrics <song name>`"); return
        else:
            query = " ".join(m.command[1:])
        proc = await m.reply_text("🔎 Fetching lyrics…")
        try:
            # Use a simple API (lyrics.ovh)
            r = await asyncio.get_event_loop().run_in_executor(
                None, lambda: requests.get(f"https://api.lyrics.ovh/v1/{quote(query)}", timeout=15)
            )
    ...
    async def _ping(c, m: Message):
        ...
        try: await msg.edit_text(f"🏓 **Pong!** `{ms:.0f}ms`\n🎵 Music Userbot Alive 🔥")
        except: pass
    ...
    async def _restart_inline(c, m: Message):
        ...
        if ok:
            try: await msg.edit_text("✅ Music userbot restarted successfully!")
            except: pass
        else:
    ...
    async def _clean(c, m: Message):
        ...
        await m.reply_text(f"🧹 Cleaned **{cleaned}** temp files.")
    ...
    handlers_to_add = [
        (_play,            ["play", "p"]),
        (_vplay,           ["vplay", "vp"]),
        (_playforce,       ["playforce", "pf"]),
        (_vplayforce,      ["vplayforce", "vpf"]),
        (_stop_cmd,        ["stop", "end"]),
        (_skip,            ["skip", "next"]),
        (_pause,           ["pause"]),
        (_resume,          ["resume"]),
    ...
    # Register stream-end event handler for auto-next-from-queue
    calls = music_calls.get(host_id)
    if calls:
...
async def restart_music_userbot(host_id: int) -> bool:
    ...
    try:
        ...
        # Start PyTgCalls FIRST
        calls = PyTgCalls(c)
        await calls.start()

        # Then store + register handlers
...
async def restore_sessions():
    ...
    if sessions:
        # Restore in parallel (limited concurrency to avoid OOM at startup)
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🧹 BACKGROUND CLEANUP TASKS (anti-OOM)
# ═══════════════════════════════════════════════════════════════════════════════
def _clean_downloads_dir(force: bool = False):
    """Clean old files from downloads dir. Returns count cleaned."""
    cleaned = 0
    try:
        total_size = 0
        files = []
        ...
        # If under limit and not forced, only delete files older than TTL
        if not force and total_size < MAX_DOWNLOAD_DIR_MB * 1024 * 1024:
            now = time.time()
            for fp, mtime, _ in files:
                if (now - mtime) > RECENT_DL_TTL:
                    # Check if file is still in use by any active stream
                    in_use = False
                    ...
                    if in_use: continue
                    try:
                        os.remove(fp); cleaned += 1
                    except: pass
        else:
            # Force-clean oldest files
        ...
        logger.error(f"clean_downloads_dir err: {e}")
    return cleaned


async def background_janitor():
    """Periodic cleanup + GC to prevent OOM."""
    while True:
        try:
            ...
            # Clear old user states (idle > 30 min)
            ...
            if cleaned or collected:
                logger.info(f"🧹 Janitor: cleaned {cleaned} files, gc {collected} objects, cache size {len(RECENT_DL_CACHE)}")
        except Exception as e:
...
async def restartall_cmd(c, m: Message):
    if not is_owner(m.from_user.id):
        await m.reply_text("❌ Owner only command."); return
    ...
    # Parallel restart (with concurrency limit)
    sem = asyncio.Semaphore(3)
    results = {"ok": 0, "fail": 0}
    async def _do(h):
        async with sem:
...
async def cleancache_cmd(c, m: Message):
    ...
    gc.collect()
    await m.reply_text(f"🧹 Cleaned **{n}** files + cache cleared.")
...
async def add_sudo(client, m: Message):
    ...
    try:
        ...
        await m.reply_text("❌ Invalid user_id"); return
    sudo_users.add(uid)
    mongo_save_sudo(uid)
...
async def stop_cmd(c, m: Message):
    """Stop ONLY the streams owned by this user."""
    if not is_authorized(m.from_user.id): return
    uid = m.from_user.id
    my_streams = [sk for sk, owner in stream_owners.items() if owner == uid]
    if not my_streams:
...
async def cb_handler(c, q: CallbackQuery):
    ...
    try:
        ...
        if data == "show_help":
            await q.message.edit_text(f"```{HELP_TEXT}```",
        ...
        if data == "use_default":
            if default_account:
                ...
                await q.message.edit_text(
                    "📎 Send group info (@username or invite link or chat_id):"
                ); await q.answer(); return
            else:
                if not is_owner(uid):
                ...
                state.data["mode"] = "default"
                await q.message.edit_text(
                    "📱 Send phone number for **default account** (with country code, e.g. +91…):"
                ); await q.answer(); return
        ...
        if data == "use_custom":
            ...
            state.data["mode"] = "custom"
            await q.message.edit_text(
                "📱 Send your phone number (with country code, e.g. +91…):"
            ); await q.answer(); return
        ...
        if data == "music_host":
            ...
            await q.message.edit_text(
                "📱 Send phone number to host **music userbot** (with country code, e.g. +91…):"
            ); await q.answer(); return
        ...
        try: await q.answer(f"❌ {e}", show_alert=True)
        except: pass
...
async def _finalize_login(uid, uc: Client, mode: str, proc_msg, state):
    ...
    try: await proc_msg.edit_text("✅ Verified! Setting up your session…")
    except: pass
    ...
    if mode in ("default_otp", "default_2fa"):
        ...
        # IMPORTANT: register handlers AFTER calls are ready
    ...
    state.step = None
    state.data.pop("client", None)  # drop reference, but DON'T disconnect
    logger.info(f"✅ Finalize complete for {uid} (mode={mode})")


# ═══════════════════════════════════════════════════════════════════════════════
# 💬 TEXT MESSAGE HANDLER (login + VC Fight)
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    try:
        if state.step in ("default_phone", "custom_phone", "music_phone"):
            ...
            try:
                ...
                await proc.edit_text(
                    "📨 **OTP Sent!**\n\n"
                    "Send the OTP code as text here.\n"
        ...
        if state.step in ("default_group", "custom_group"):
            ...
            if not ci:
                await m.reply_text("❌ Invalid input. Send @username, https://t.me/+xxx, or chat_id")
                return
...
async def on_startup():
    ...
    logger.info("✅ Background janitor started")

def _banner():
    print("\n" + "="*68)
    print("🔥  ZUDO USERBOT — GOD-LEVEL EDITION v8 (ULTRA-STABLE + 50+ CMDS)")
    print("="*68)
    ...
    print(f"   ⚡ Stream Mode   : DOWNLOAD-FIRST (8x parallel)")
    ...
    print(f"   🧹 Janitor       : every {GC_INTERVAL_SEC}s (OOM protection)")
...
if __name__ == "__main__":
    _banner()
    logger.info("🚀 Starting ZUDO USERBOT v8 …")
