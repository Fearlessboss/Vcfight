"""
🔥 ZUDO USERBOT — GOD-LEVEL EDITION v2 (FIXED + UPGRADED)
==================================================================
✦ FIX: .play ab turant VC join karta aur play karta hai (instant)
✦ FIX: 2FA verification real-time hota hai, login ke turant baad confirmation
✦ FIX: BOT_TOKEN ab environment variable se aata hai (hardcoded nahi)
✦ FIX: 2 owners support (primary + co-owner)
✦ FIX: Music handlers properly attach hote hain — .play / .skip / .ping kaam karte hain
✦ FIX: Peer-id invalid — auto warm-up dialogs after login/restore + retry
✦ FIX: Better logs — har step pe clear info
✦ FIX: Search → Download → Join VC ko parallel optimize kiya
✦ FIX: Music host concurrency lock — race condition khatam
✦ MongoDB persistent memory (sessions, sudo, cache, hosts)
✦ Loud audio boost (4x default — speaker phaadne wala)
✦ /help & /commands — full command list

Powered by @zudo_userbot
==================================================================
ENV VARS REQUIRED:
    BOT_TOKEN        — Telegram bot token (REQUIRED, no default in code)
    API_ID           — optional, defaults to bundled value
    API_HASH         — optional, defaults to bundled value
    MONGO_URL        — optional, defaults to bundled value
    OWNER_ID         — optional, primary owner (default 7661825494)
    CO_OWNER_ID      — optional, secondary owner (default 6980326908)
    NUBCODER_TOKEN   — optional
    VOLUME_BOOST     — optional float, default 4.0
==================================================================
"""
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🔧 CONFIGURATION  (BOT_TOKEN is REQUIRED via env var)
# ═══════════════════════════════════════════════════════════════════════════════
...
# 🚨 BOT_TOKEN must come from environment — never hardcoded
...
# ═══════════════════════════════════════════════════════════════════════════════
# 📜 LOGGING — colored, structured, verbose
# ═══════════════════════════════════════════════════════════════════════════════
...
# silence pyrogram/pytgcalls flood unless WARNING+
...
default_calls   = None
user_accounts   = {}   # VC fight  uid -> Client
user_calls      = {}   # uid -> PyTgCalls
music_accounts  = {}   # music userbot uid -> Client
music_calls     = {}   # uid -> PyTgCalls
active_streams  = {}   # stream_key -> chat_id

# concurrency locks — prevent double join / race conditions
...
async def warmup_peers(client: Client, label: str = ""):
    """Iterate dialogs once so Pyrogram caches every peer id."""
    try:
        count = 0
...
async def download_youtube_audio(url_or_query: str):
    """Download audio. Returns (path, title) or (None, None)."""
    logger.info(f"🔎 Audio search: {url_or_query}")

    # Try nubcoder first (fast)
    ...
    if stream_url:
        try:
            ...
            if os.path.exists(output_path) and os.path.getsize(output_path) > 1000:
                logger.info(f"✅ nubcoder dl ok: {title}")
    ...
    # Fallback: yt-dlp
    try:
        output_path = f'/tmp/downloads/{int(asyncio.get_event_loop().time()*1000)}'
        ydl_opts = {
        ...
        if os.path.exists(final):
            logger.info(f"✅ yt-dlp dl ok: {info_holder.get('title')}")
...
async def auto_leave_after_playback(calls, stream_key, chat_id, audio_path):
    try:
        ...
        logger.info(f"👋 Auto-left VC ({stream_key}) after {dur}s")
    except Exception as e:
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🎯 CORE: join VC + play  (instant, with peer-warmup retry)
# ═══════════════════════════════════════════════════════════════════════════════
async def join_vc_and_play(client: Client, calls: PyTgCalls,
                           chat_id: int, audio_path: str, stream_key) -> tuple[bool, str | None]:
    """
    Returns (success, err_msg). Joins VC instantly, retries once with peer warmup.
    """
    async def _try_join():
...
━━━━━━━ 🎵  MUSIC USERBOT  (in groups, dot-prefix)  ━━━━━━━
  .play   <song / yt url>     • Play track INSTANTLY
...
def register_music_handlers(client: Client, host_id: int):
    """Attach all `.play` etc handlers using add_handler (works on restored sessions)."""
    ...
    async def _play(c, m: Message):
        try:
            ...
            if lock.locked():
                await m.reply_text("⏳ Already preparing a track for this chat, please wait…")
                return
            ...
            async with lock:
                ...
                try:
                    await proc.edit_text(f"🔎 Searching: `{query}`\n⬇️ Downloading audio…")
                except: pass
                ...
                # Step 2: Join VC INSTANTLY
                try:
                    await proc.edit_text(f"🎵 `{title}`\n🔌 Joining voice chat…")
                except: pass

                ok, err = await join_vc_and_play(c, calls, chat_id, audio_path, f"music_{host_id}")
                if not ok:
                ...
                asyncio.create_task(auto_leave_after_playback(
                    calls, f"music_{host_id}", chat_id, audio_path))
                asyncio.create_task(cleanup_file(audio_path))
                ...
                logger.info(f"🎵 Playing '{title}' in chat {chat_id} via host {host_id}")
        except Exception as e:
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🔑 OTP / 2FA VERIFY  (instant, no fake "still verifying" delays)
# ═══════════════════════════════════════════════════════════════════════════════
async def verify_otp(cq: CallbackQuery, mode: str, otp: str):
    ...
    try:
        ...
        # No 2FA needed — finalize now
        await _finalize_login(uid, uc, mode, proc, state)
    except SessionPasswordNeeded:
        # 2FA required — switch state, ask for password
        state.step = {
        ...
        await proc.edit_text(
            ...
            "_(Verification will complete instantly once you send it.)_"
        )
...
async def _finalize_login(uid, uc: Client, mode: str, proc_msg, state):
    """
    INSTANT finalization: as soon as Telegram confirms login, save + warm + announce.
    Real-time, no fake "still verifying" stall.
    """
    global default_account, default_calls
    ...
    # Warm peers in background so UI doesn't stall
...
@bot.on_message(filters.private & filters.text & ~filters.command(
    ["start","help","commands","sudo","rmsudo","sudolist","stop","status","logout","owners"]))
async def msg_handler(client, m: Message):
    ...
    try:
        ...
        if state.step == "audio_input":
            ...
            await proc.edit_text("⬇️ Downloading & boosting audio... 🔊")
            ...
            ok, err = await join_vc_and_play(client_to_use, calls_to_use, actual_id, audio_path, stream_key)
            if ok:
...
async def audio_handler(client, m: Message):
    ...
    ok, err = await join_vc_and_play(cu, calls, actual, boosted, sk)
    if ok:
...
def _banner():
    print("\n" + "="*68)
    print("🔥  ZUDO USERBOT — GOD-LEVEL EDITION v2")
    print("="*68)
...
if __name__ == "__main__":
    _banner()
    logger.info("🚀 Starting ZUDO USERBOT v2 …")
