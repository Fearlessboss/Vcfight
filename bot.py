"""
🔥 ZUDO USERBOT — GOD-LEVEL EDITION v6 (DOWNLOAD-FIRST + INSTANT PLAY)
==================================================================
✦ FIX: "Timeout fetching stream" — ab yt-dlp DIRECTLY audio download karta hai
       (raw googlevideo URL nahi, jo expire/slow hota hai)
✦ FIX: Local file path pytgcalls ko deta hai → INSTANT VC join + play
✦ FIX: Parallel peer-warmup + download = total ~5-8 sec se play start
✦ NEW: Auto cleanup of downloaded files
✦ KEEP: Multi-client yt-dlp rotation, MediaStream/AudioPiped fallback,
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🎵 DOWNLOAD-FIRST RESOLVER — yt-dlp DOWNLOADS audio file locally (not URL!)
# ═══════════════════════════════════════════════════════════════════════════════
...
def _make_dl_opts(idx: int, out_template: str):
    """yt-dlp options for DOWNLOADING audio to disk (small m4a, fast)."""
    opts = {
        # bestaudio under 5MB if possible, otherwise just bestaudio — small files = fast download
...
def _sync_download_yt(query: str):
    """
    Download audio to local file via yt-dlp with multi-client rotation.
    ...
    for idx in range(len(_YT_PLAYER_CLIENTS)):
        try:
            opts = _make_dl_opts(idx, out_template)
            ...
            file_path = None
            # try _filename / requested_downloads
            ...
            logger.info(f"✅ yt-dlp[{idx}/{_YT_PLAYER_CLIENTS[idx]}] downloaded '{title}' ({size_mb:.1f}MB) → {file_path}")
            return {
                'file_path': file_path,
                'title': title,
                'duration': duration,
...
async def resolve_stream_instant(query: str, want_video: bool = False):
    """
    DOWNLOAD audio file to disk and return its local path.
    Returns (file_path, title) or (None, None).
    """
    logger.info(f"⚡ Resolving + downloading: {query}")
    loop = asyncio.get_event_loop()

    try:
        info = await loop.run_in_executor(None, lambda: _sync_download_yt(query))
        if info and info.get('file_path'):
            return info['file_path'], info.get('title', 'Unknown')
    except Exception as e:
...
async def auto_leave_after_playback(calls, stream_key, chat_id, source, cleanup_path=None):
    try:
...
async def join_vc_and_stream(client: Client, calls: PyTgCalls,
    ...
    # If already in VC → switch stream
...
def register_music_handlers(client: Client, host_id: int):
    ...
    async def _play(c, m: Message):
        try:
            ...
            async with lock:
                proc = await c.send_message(chat_id, f"⚡ Downloading: `{query}`")
                t0 = time.time()

                # parallel: download + ensure peer
                resolve_task = asyncio.create_task(resolve_stream_instant(query))
                asyncio.create_task(ensure_peer(c, chat_id))

                try:
                    file_path, title = await asyncio.wait_for(resolve_task, timeout=45)
                except asyncio.TimeoutError:
                ...
                if not file_path:
                    try: await proc.edit_text("❌ Couldn't download audio. Try another query.")
                    except: pass
                    return
                ...
                ok, err = await join_vc_and_stream(
                    c, calls, chat_id, file_path, f"music_{host_id}",
                ...
                asyncio.create_task(auto_leave_after_playback(
                    calls, f"music_{host_id}", chat_id, file_path, cleanup_path=file_path))
                ...
                logger.info(f"🎵 Streaming '{title}' in chat {chat_id} via host {host_id} ({elapsed:.1f}s)")
        except Exception as e:
...
async def msg_handler(client, m: Message):
    ...
    try:
        ...
        if state.step == "audio_input":
            ...
            try:
                file_path, song_title = await asyncio.wait_for(resolve_task, timeout=45)
            except asyncio.TimeoutError:
            ...
            ok, err = await join_vc_and_stream(
                client_to_use, calls_to_use, actual_id, file_path, stream_key,
                is_video=False, host_id=uid if mode != "default" else "default"
            )
            elapsed = time.time() - t0
            if ok:
                asyncio.create_task(auto_leave_after_playback(
                    calls_to_use, stream_key, actual_id, file_path, cleanup_path=file_path))
                await proc.edit_text(
                    f"✅ **NOW STREAMING** 🔥\n\n📻 Group: {title}\n🎵 {song_title or 'Audio'}\n"
                    f"⚡ Started in {elapsed:.1f}s\n\nUse /stop to stop")
            else:
...
async def audio_handler(client, m: Message):
    ...
    if ok:
        asyncio.create_task(auto_leave_after_playback(calls, sk, actual, boosted, cleanup_path=boosted))
...
def _banner():
    print("\n" + "="*68)
    print("🔥  ZUDO USERBOT — GOD-LEVEL EDITION v6 (DOWNLOAD-FIRST)")
    print("="*68)
    ...
    print(f"   ⚡ Stream Mode   : DOWNLOAD-FIRST (yt-dlp → local file → pytgcalls)")
...
if __name__ == "__main__":
    _banner()
    logger.info("🚀 Starting ZUDO USERBOT v6 …")
